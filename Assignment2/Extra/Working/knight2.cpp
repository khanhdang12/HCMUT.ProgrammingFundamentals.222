...
{
    bool Omega = false, HadesBool = false, choose = false;
    ...
    for (int i = 0; i < len; i++)
    {
        ...
        else if (thisEvent == 99)
        {
            ...
            for (int i = sizeArmy - 1; i >= 0; i--)
            {
                if ((arrArmy + i)->getKnightType() == NORMAL)
                {
                    temp = (arrArmy + i);
                    break;
                }
            }
            if (fight(opponent) && temp != nullptr)
            {
                string s = "No. knights: " + to_string(this->count());
                if (this->count() > 0)
                {
                    BaseKnight *lknight = temp;
                    s += ";" + lknight->toString();
                }
                s += ";PaladinShield:" + to_string(this->hasPaladinShield());
                s += ";LancelotSpear:" + to_string(this->hasLancelotSpear());
